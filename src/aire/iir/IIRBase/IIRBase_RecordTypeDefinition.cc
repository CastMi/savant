...
void 
IIRBase_RecordTypeDefinition::set_resolution_function( IIR_FunctionDeclarationRef resolution_function ){
  my_resolution_function = resolution_function;
}

// List Accessor(s)
IIR_ElementDeclarationListRef
IIRBase_RecordTypeDefinition::get_element_declarations() {
  ASSERT(element_declarations != nullptr);
  return element_declarations;
}


void
IIRBase_RecordTypeDefinition::set_element_declarations(IIR_ElementDeclarationListRef new_element_declarations) {
  ASSERT(new_element_declarations != NULL);
  element_declarations = new_element_declarations;
}

IIRRef
IIRBase_RecordTypeDefinition::convert_tree(plugin_class_factoryRef factory) {
  // Get the node itself
  IIRBase_RecordTypeDefinitionRef new_node = my_dynamic_pointer_cast<IIRBase_RecordTypeDefinition>(IIRBase_TypeDefinition::convert_tree(factory));

  // Process the variables
  new_node->element_declarations = my_dynamic_pointer_cast<IIR_ElementDeclarationList>(convert_node(element_declarations, factory));
  ...
  if (my_resolution_function != NULL)
    ...
    new_node->my_resolution_function = IIR_FunctionDeclarationRef();
...
IIRBase_RecordTypeDefinition::get_element_subtype( int index ){
  IIR_TypeDefinitionRef retval;

  ASSERT( index <= get_element_declarations()->num_elements() );

  IIR_ElementDeclarationRef element_decl = NULL;
  
  element_decl = my_dynamic_pointer_cast<IIR_ElementDeclaration>(get_element_declarations()->get_nth_element( index ));
...
IIRBase_RecordTypeDefinition::is_locally_static(){
  ...
  IIR_ElementDeclarationRef current_decl = my_dynamic_pointer_cast<IIR_ElementDeclaration>(get_element_declarations()->first());
  while( current_decl != NULL ){
    ...
    current_decl = my_dynamic_pointer_cast<IIR_ElementDeclaration>(get_element_declarations()->successor( current_decl ));
  }
...
IIRBase_RecordTypeDefinition::find_declarations( IIR_NameRef to_find ){
