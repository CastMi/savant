   ...
   if( prefix_decls == NULL ){
      ...
      for(auto it = prefix_decls->begin(); it != prefix_decls->end(); it++) {
         ...
         if( suffix_decls != NULL ){
            ...
            retval->insert( suffix_decls );
            if( found_in_library == TRUE ){
               (*it)->_add_to_declarative_region( suffix_decls );
            }
         }      
      }
   }
...
IIRScram_SelectedName::_decl_to_decl( IIRScram_Declaration *suffix_decl ){
   ...
   if( get_suffix()->get_kind() == IIR_DESIGNATOR_BY_ALL ){
      for(auto it = prefix_decls->begin(); it != prefix_decls->end(); it++) {
         if( *it == suffix_decl ){
            set_prefix( _get_prefix()->_decl_to_decl( *it ) );
            break;
         }
      }
   }
      ...
      for(auto it_ext = prefix_decls->begin(); it_ext != prefix_decls->end(); it_ext++) {
         ...
         savant::set<IIR_Declaration*>      *temp_set 
            = (*it_ext)->find_declarations( &temp_name );
         ...
         if( found != NULL ){
            for(auto it_in = found->begin(); it_in != found->end(); it_in++) {
               if( *it_in == suffix_decl ){
                  set_prefix( _get_prefix()->_decl_to_decl( *it_ext ) );
                  break;
               }
            }
            delete found;
