...
IIRScram_DesignatorByAll::_get_rval_set( savant::set<IIRScram_Declaration*> *prefix_decls,
   ...
   for(auto it = prefix_decls->begin(); it != prefix_decls->end(); it++) {
      if( (*it)->is_access_type() == TRUE ){
         IIRScram_TypeDefinition *access_to = (*it)->_get_subtype()->_get_designated_subtype();
         retval->insert( access_to );
      }
   }
...
IIRScram_DesignatorByAll::_get_rval_set( savant::set<IIRScram_TypeDefinition*> *prefix_types,
   ...
   for(auto it = prefix_types->begin(); it != prefix_types->end(); it++) {
      if( (*it)->is_access_type() == TRUE ){
         IIRScram_TypeDefinition *access_to = (*it)->_get_designated_subtype();
         retval->insert( access_to );
      }
   }
