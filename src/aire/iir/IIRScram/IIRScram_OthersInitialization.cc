...
IIRScram_OthersInitialization::_build_element_subtype_set( savant::set<IIRScram_TypeDefinition*> *context_set ){
   ...
   for(auto it = context_set->begin(); it != context_set->end(); it++) {
      if( (*it)->is_array_type() == TRUE ){
         retval->insert( (*it)->_get_element_subtype() );
      }
      else{
...
IIRScram_OthersInitialization::_get_rval_set( constraint_functor * ){
   ...
   for(auto it_ext = array_types->begin(); it_ext != array_types->end(); it_ext++) {
      if( (*it_ext)->_get_element_subtype() != NULL ){
      }

      bool one_matched = false;
      for(auto it_in = actual_rvals->begin(); it_in != actual_rvals->end(); it_in++) {
         if( (*it_in)->is_compatible( (*it_ext)->_get_element_subtype() ) != NULL ){
            one_matched = true;
         }
      }
