   ...
   if( my_decls == NULL ){
      ...
      if( get_kind() != IIR_USER_ATTRIBUTE ){
         ...
         if(my_decls->size() == 0) {
            ...
            my_decls->insert(attribute_decl);
         }
         //$$$ - end of my code 
      }
   }
...
IIRScram_Attribute::_resolve_prefix(){
   ...
   if( done == false ){
      ...
      /*
       * The size of prefix_rvals is exactly
       * 1 here (take a look to the previous if)
       */
      ASSERT( prefix_rvals->size() == 1 );
      _get_prefix()->_type_check( prefix_rvals );  
      set_prefix( _get_prefix()->_rval_to_decl( *(prefix_rvals->begin())) );
...
IIRScram_Attribute::_resolve_suffix_base_type_of_prefix(){
   ...
   if( _get_suffix() == NULL || _get_suffix()->is_resolved() == FALSE ){
      ...
      for(auto it = suffix_types->begin(); it != suffix_types->end(); it++) {
         if( (*it)->is_compatible( prefix_type ) ){
            current_type = *it;
            break;
         }
      }
...
IIRScram_Attribute::_resolve_suffix_local_static_int(){
   ...
   if( _get_suffix() != NULL && _get_suffix()->is_resolved() == FALSE ){
      ...
      IIRScram_TypeDefinition *current_type = NULL;
      for(auto it = suffix_types->begin(); it != suffix_types->end(); it++) {
         if( *it == dynamic_cast<IIRScram_TypeDefinition *> (package->get_savant_universal_integer()) ){
            current_type = *it;
            break;
         }
      }
   ...
   if( prefix_rvals != NULL ){
      ...
      for(auto it = prefix_rvals->begin(); it != prefix_rvals->end(); it++) {
         if( (*it)->is_array_type() == TRUE ){
            retval->insert( (*it)->_get_resolved_index_subtype() );
         }
         else if ( (*it)->is_scalar_type() == TRUE ){
            retval->insert( *it );
         }
      }
   }
