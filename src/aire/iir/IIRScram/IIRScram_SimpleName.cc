...
IIRScram_SimpleName::_symbol_lookup() {
   ...
   if( prefix->_is_iir_declaration() == true ){
      retval = new savant::set<IIRScram_Declaration*>( dynamic_cast<IIRScram_Declaration *>(prefix) );
   }
   else if( prefix->is_text_literal() == TRUE ){
      savant::set<IIR_Declaration*> *decls= new savant::set<IIR_Declaration*>(*_get_symbol_table()->find_set( get_prefix_string()));
      retval = decls->convert_set<IIRScram_Declaration*>();
      delete decls;
      if( IIRBase_TextLiteral::cmp( get_prefix_string(), "work" ) == 0 ){
         retval->insert( _get_work_library() );
      }
   }
   else{
...
savant::set<IIRScram_Declaration*> *
IIRScram_SimpleName::_symbol_lookup( savant::set<IIRScram_Declaration*> *set_to_look_in ) {
   savant::set<IIRScram_Declaration*> *retval = new savant::set<IIRScram_Declaration*>;

   for(auto it = set_to_look_in->begin(); it != set_to_look_in->end(); it++) {
      savant::set<IIRScram_Declaration*>   *found = NULL;
      savant::set<IIR_Declaration*>        *temp_set 
         = (*it)->find_declarations( this );
...
void 
IIRScram_SimpleName::_type_check( savant::set<IIRScram_TypeDefinition*> * ){}
...
IIRScram_SimpleName::_rval_to_decl(IIRScram_TypeDefinition *my_rval) {
   ...
   savant::set<IIRScram_Declaration*> *possibilities = new savant::set<IIRScram_Declaration*>;
   for(auto it = my_decls->begin(); it != my_decls->end(); it++) {
      if ( ((*it)->_get_subtype() != NULL  
               && (*it)->_get_subtype()->is_compatible( my_rval ) != NULL ) && 
            ((*it)->is_subprogram() == FALSE || 
             (dynamic_cast<IIRScram_SubprogramDeclaration *>(*it))->_num_required_args() == 0 )) {
...
IIRScram_SimpleName::_determine_decl_in_set( savant::set<IIRScram_Declaration*> *search_in,
      IIRScram_TypeDefinition *looking_for ){
   for(auto it_ext = search_in->begin(); it_ext != search_in->end(); it_ext++) {
      savant::set<IIRScram_Declaration*>   *decls = NULL;
      savant::set<IIR_Declaration*>        *temp_set 
