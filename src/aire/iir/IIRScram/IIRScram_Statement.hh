...
REF_FORWARD_DECL(IIRScram_CaseStatementAlternativeList);
REF_FORWARD_DECL(IIRScram_WaveformList);
...
public:
  ...
  static IIRScramRef _type_check_and_resolve_boolean_condition( IIRScramRef condition );
	
  // These virtual methods are needed to support the type checking functions in the
  // protected section
  virtual IIRScramRef _get_target();
  virtual IIR_DelayMechanism _get_delay_mechanism();
  virtual IIRScramRef _get_reject_time_expression();
  ...
  virtual IIRScramRef _get_case_statement_expression();
  virtual void _set_case_statement_expression( IIRScramRef  );

  /** For assertion statements this method is overloaded to return the
      assertion condtion.  For anything else it returns 0. */
  virtual IIRScramRef _get_assertion_condition(){ return 0; }
  ...
  virtual IIRScramRef _get_report_expression(){ return 0; }

  /** For report and assertion statements this method is overloaded to
      return the report expression. For anything else it returns 0. */
  virtual IIRScramRef _get_severity_expression(){ return 0; }
  ...
  IIRScram_LabelRef _get_label() const;

  /**
     Returns the library unit that contains this statement.
  */
  IIR_LibraryUnitRef _get_design_unit();
...
protected:
  ...
  void _type_check_target_and_waveform( IIRScram_WaveformListRef  );
  void _type_check_mechanism_and_time( IIRScram_WaveformListRef  );
  ...
  void _type_check_case_statement_expression( IIRScram_CaseStatementAlternativeListRef  );
