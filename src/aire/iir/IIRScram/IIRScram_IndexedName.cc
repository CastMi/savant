...
IIRScram_IndexedName::_my_type_given_array_prefix_type( IIRScram_TypeDefinition *prefix_rval ){
   ...
   if( _get_suffix()->_is_iir_type_definition() == TRUE && _get_suffix()->is_scalar_type() == TRUE ){
      // This is a slice.
      retval->insert(prefix_rval);
      if( prefix_rval->is_access_type() == TRUE ){
         retval->insert( (dynamic_cast<IIRScram_AccessTypeDefinition *>(prefix_rval))->_get_designated_type() );
      }
   }
      ...
      if( _get_suffix()->get_kind() == IIR_RANGE_ATTRIBUTE ||
         ...
         // Not slice
         retval->insert( prefix_rval->_get_type_of_element( get_num_indexes() ) );
      }
   }
      ...
      if( suffix_decls != NULL ){
         ...
         retval->insert( prefix_rval );
         retval->insert( prefix_rval->_get_type_of_element( prefix_rval->get_num_indexes() ) );
      }
      delete suffix_decls;
   }
   else{
...
IIRScram_IndexedName::_symbol_lookup( savant::set<IIRScram_Declaration*> *look_in ){
   ...
   for(auto it = look_in->begin(); it != look_in->end(); it++) {
      if( (*it)->find_declarations( this ) != NULL ){
         retval->insert( (*it)->find_declarations( this)->convert_set<IIRScram_Declaration*>() );
      }
   }
      ...
      case IIR_Declaration::FUNCTION:
         {
            ...
            // 	index_num = 1;
            //       }
            //       retval->insert( current_rval->_get_type_of_element( index_num ) );	  
            //     }
            //     else{
            ...
            if( add_rval == TRUE ){
               retval->insert( prefix_decl->_get_subtype() );
            }
            break;
         }
                  ...
                  if( prefix_decls != NULL ){
                     ...
                     for(auto it = prefix_decls->begin(); it != prefix_decls->end(); it++) {
                        savant::set<IIRScram_TypeDefinition*> *return_type = 
                           _return_type_given_prefix_declaration( *it );
                        if( return_type != NULL ){
                     ...
                     for(auto it = prefix_types->begin(); it != prefix_types->end(); it++) {
                        if( (*it)->is_array_type() == TRUE ){
                           // Then it's an array access or a slice...  It's not a function call, 
                           savant::set<IIRScram_TypeDefinition*> *tmp = _my_type_given_array_prefix_type( *it );
                           retval->insert( tmp );
                           delete tmp;
                        }
                        else{
         ...
         savant::set<IIRScram_TypeDefinition*> *
            IIRScram_IndexedName::_get_rval_set( savant::set<IIRScram_Declaration*> *look_in,
               ...
               for(auto it_ext = look_in->begin(); it_ext != look_in->end(); it_ext++) {
                  ...
                  if ((*it_ext)->find_declarations( dynamic_cast<IIRScram_Name *>(_get_prefix()) ) != NULL) {
                     prefix_decls = (*it_ext)->find_declarations( dynamic_cast<IIRScram_Name *>(_get_prefix()) )->convert_set<IIRScram_Declaration*>();
                  }
                  if( prefix_decls != NULL ){
                     for(auto it_in = prefix_decls->begin(); it_in != prefix_decls->end(); it_in++) {
                        savant::set<IIRScram_TypeDefinition*> *return_type;
                        return_type = _return_type_given_prefix_declaration( *it_in );
                        if( return_type != NULL ){
                           retval->insert( return_type );
                        }
                     }
                  ...
                  switch ( func_decls->size() ){
                     ...
                     case 1:{
                               ...
                               IIRScram_SubprogramDeclaration *subprogram_decl = 
                                  dynamic_cast<IIRScram_SubprogramDeclaration *>(*(func_decls->begin())); 
            ...
            IIRScram_IndexedName::_process_as_slice( savant::set<IIRScram_TypeDefinition*> *context_set ){
               ...
               for(auto it_ext = prefix_rvals->begin(); it_ext != prefix_rvals->end(); it_ext++) {
                  ...
                  if( (*it_ext)->is_access_type() == TRUE ){
                     to_consider = (dynamic_cast<IIRScram_AccessTypeDefinition *>(*it_ext))->_get_designated_type();
                  }
                  else{
                     to_consider = *it_ext;
                  }
                  ...
                  for(auto it_in = context_set->begin(); it_in != context_set->end(); it_in++) {
                     IIRScram_TypeDefinition *current_context_type = dynamic_cast<IIRScram_TypeDefinition *>(*it_in);
  ...
  IIRScram *orig_suffix = _get_suffix();
         ...
         IIRScram *
            IIRScram_IndexedName::_rval_to_decl( IIRScram_Declaration *declaration, IIRScram_TypeDefinition *my_rval ){
               ...
               if( prefix_decls != NULL ){
                  for(auto it = prefix_decls->begin(); it != prefix_decls->end(); it++) {
                     savant::set<IIRScram_TypeDefinition*> *return_set = _return_type_given_prefix_declaration( *it );
                     ...
                     if( return_set != NULL ){
                        ...
                        if( return_type == my_rval ){
                           ...
                           if( _get_suffix()->is_resolved() == FALSE ){
                              ...
                              IIRScram_TypeDefinition *prefix_rval = (*it)->_get_subtype();
                              ASSERT( prefix_rval != NULL );
         ...
         IIRScram_Declaration *
            IIRScram_IndexedName::_determine_decl_in_set( savant::set<IIRScram_Declaration*> *look_in,
               ...
               for(auto it_ext = look_in->begin(); it_ext != look_in->end(); it_ext++) {
                  ...
                  if( prefix_decls != NULL ){
                     for(auto it_in = prefix_decls->begin(); it_in != prefix_decls->end(); it_in++) {
                        savant::set<IIRScram_TypeDefinition*> *return_set = _return_type_given_prefix_declaration( *it_in );
                        ASSERT( return_set == NULL || return_set->size() == 1 );	  
                        if( return_set != NULL ){
                           IIRScram_TypeDefinition *return_type = *(return_set->begin());
                           if( return_type == my_rval ){
