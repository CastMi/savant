...
library_manager::parse_primary_unit( IIR_LibraryDeclaration     *lib_decl,
   ...
   if( unit_name->get_suffix() == NULL || 
      ...
      if( entity_decl != NULL ){
         decls.insert( entity_decl );
      }
      if( package_decl != NULL ){
         decls.insert( package_decl );
      }
      if( configuration_decl != NULL ){
         decls.insert( configuration_decl );
      }
      switch( decls.size() ){
         case 0:{
...
library_manager::find_library( IIR_TextLiteral *library_name,
   ...
   {
      // TODO: Remove Scram
      if( IIRBase_TextLiteral::cmp( library_name, (*it)->get_declarator() ) == 0 ){
         retval = *it;
         break;
      }
   }
...
library_manager::lookup_unit( IIR_Boolean               complain_on_error,
   ...
   {
      ASSERT( dynamic_cast<IIR_LibraryUnit *>(*it) );
      IIR_LibraryUnit *as_lib_unit = dynamic_cast<IIR_LibraryUnit *>(*it);
      IIR_LibraryDeclaration *its_lib = as_lib_unit->get_library();
      if( (*functor)(*it) && its_lib == target_library ){
         retval = dynamic_cast<IIR_LibraryUnit *>(*it);
         break;
      }
   }
...
library_manager::lookup_architecture( IIR_Boolean               complain_on_error, 
   ...
   if( decls != NULL && decls->size() > 0 ){
      ...
      for(auto it = decls->begin(); it != decls->end(); it++)
      {
         ASSERT( *it && dynamic_cast<IIR_ArchitectureDeclaration *>(*it) );
         current = *it;
      }
      delete decls;
...
library_manager::get_libraries(){
   return new savant::set<IIR_LibraryDeclaration*>( *libraries );
}
