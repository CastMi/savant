...
symbol_lookup::build_hidden_declaration_set( IIR_Declaration *about_to_add ){
   ...
   for(auto it = current_declarations->begin(); it != current_declarations->end(); it++)
   {
      ...
      IIRScram_Declaration *current_declaration = dynamic_cast<IIRScram_Declaration *>(*it);
      ...
      if( are_homographs == TRUE ){
         ...
         retval->insert( current_declaration );
      }
...
symbol_lookup::lookup_add( IIR_Declaration *decl_ptr) {
   ...
   current_declarations->insert(decl_ptr);
   ...
   if( need_to_hide != NULL ){
      ...
      if( debug_symbol_table ){
         ...
         {
            IIRBase_Declaration *current = dynamic_cast<IIRBase_Declaration *>(*it);
         ...
         if( hidden_info != NULL ){
            ...
            // TODO: Remove Scram
            for(auto it = hidden_info->get_hidden_declarations()->begin();
                  it != hidden_info->get_hidden_declarations()->end(); it++)
            {
               IIRBase_Declaration *current = dynamic_cast<IIRBase_Declaration*>(*it);
               current->publish_vhdl_decl( cerr );
               cerr << " (" << current->get_kind_text() << " *)" << current <<
...
symbol_lookup::lookup_remove( IIR_Declaration *decl_ptr ){
   ...
   if( hidden_info != NULL ){
            ...
            it != hidden_info->get_hidden_declarations()->end(); it++)
      {
         if( debug_symbol_table ){
            cerr << this <<  " - Unhiding:\n";
            dynamic_cast<IIRBase_Declaration *>( *it )->publish_vhdl_decl( cerr );
            cerr << " : ( " << *it;
            cerr << " node type : " << (*it)->get_kind_text() << ") " << endl;
         }
         decl_set->insert( *it );
      }
...
symbol_lookup::find_hidden_symbol_entry( IIR_Declaration *to_find, 
   ...
   for(auto it = chain->hidden_declaration_info.begin();
         it != chain->hidden_declaration_info.end(); it++)
   {
      if( (*it)->get_declaration() == to_find ){
         // FIXME very very bad
         retval = *it;
         break;
      }
...
symbol_lookup::update_hidden_symbol_entry( IIR_Declaration *to_update, 
   ...
   if( hidden_decls != NULL ){
      ...
      chain->hidden_declaration_info.insert( new_entry );
   }
   else{
      // to_update has gone out of scope and we need to clear it's hidden info.
      chain->hidden_declaration_info.erase( hidden_entry );
      delete hidden_entry;
   }
}
