...
decl_set_to_typedef_set(savant::set<IIRScram_Declaration*> *decl_set,constraint_functor *functor ){
   savant::set<IIRScram_TypeDefinition*> *typedef_set = NULL;

   if( decl_set != NULL ){
      int size = decl_set->size();

      if( size > 0 ){
         typedef_set = new savant::set<IIRScram_TypeDefinition*>;
         ...
         for(auto it = decl_set->begin(); it != decl_set->end(); it++)
         {
            current_rval = (*it)->_get_subtype();
            if( current_rval != NULL ){
               typedef_set->insert( current_rval );
            }
         }
      }
      // else nothing..
...
reconcile_sets( savant::set<IIRScram_TypeDefinition*> *lval_set, 
   ...
   savant::set<IIRScram_TypeDefinition*> new_rval_set;

   bool one_matched = false;
   for(auto it_ext = lval_set->begin(); it_ext != lval_set->end(); it_ext++)
   {
      one_matched = false;
      for(auto it_int = rval_set->begin(); it_int != rval_set->end(); it_int++)
      {
         IIRScram_TypeDefinition *compatible_subtype = dynamic_cast<IIRScram_TypeDefinition *>((*it_ext)->is_compatible( *it_int ));
         if( compatible_subtype != NULL ){
            one_matched = true;
            new_rval_set.insert( *it_int );
         }
      }
      if( one_matched == false ){
         lval_set->erase( it_ext );
      }
   }
...
reconcile_sets( savant::set<IIRScram_Declaration*> *decls, 
   ...
   savant::set<IIRScram_Declaration*> original_set( *decls );

   bool one_matched = false;
   // First try to do an exact subtype match...
   for(auto it = decls->begin(); it != decls->end(); it++)
   {
      if( (*it)->_get_subtype() != NULL ){
         if( rval_set->contains( (*it)->_get_subtype() ) ){
            one_matched = true;
         }
         else {
            decls->erase( *it );
   ...
   if( one_matched == false ){
      *decls = original_set;
      ...
      for(auto it_ext = decls->begin(); it_ext != decls->end(); it_ext++)
      {
         if( (*it_ext)->_get_subtype() != NULL ){
            one_matched = false;
            for(auto it_int = rval_set->begin(); it_int != rval_set->end(); it_int++)
            {
               if( (*it_ext)->_get_subtype()->is_compatible( *it_int ) != NULL ){
                  one_matched = TRUE;
               }
            }
   ...
   {
      if( (*it)->_get_subtype() != NULL ){
         rval_set->insert( (*it)->_get_subtype() );
      }
   }
   // Nothing to return - the sets are built!
}
...
resolve_subprogram_decls(savant::set<IIRScram_Declaration*>              *possible_subprogram_decls,
      ...
      const savant::set<IIRScram_TypeDefinition*>     *possible_return_types){
   ...
   for(auto it = possible_subprogram_decls->begin();
         it != possible_subprogram_decls->end(); it++)
   {
      ASSERT( (*it)->is_subprogram() == TRUE );
      IIRScram_SubprogramDeclaration *as_subprog = dynamic_cast<IIRScram_SubprogramDeclaration *>(*it);
      ...
      if( arguments_match == FALSE ){
         possible_subprogram_decls->erase( *it );
      }
   }
   ...
   if( possible_return_types != NULL ){
      ...
      savant::set<IIRScram_TypeDefinition*> temp_return_types( (savant::set<IIRScram_TypeDefinition*> &)*possible_return_types );
...
void 
resolve_subprogram_decls(savant::set<IIRScram_Declaration*>                      *possible_subprogram_decls,
      dl_list<savant::set<IIRScram_TypeDefinition*> >         *parameter_rval_list,
   ...
   for(auto it_ext = possible_subprogram_decls->begin();
         it_ext != possible_subprogram_decls->end(); it_ext++)
   {
      // Go through all of the parameters, and see if one matches..
      int i = 0;

      savant::set<IIRScram_TypeDefinition*> *current_parameter_rvals;
      ...
      while( current_parameter_rvals != NULL ){
         ...
         for(auto it_int = current_parameter_rvals->begin();
               it_int != current_parameter_rvals->end(); it_int++)
         {
            if((*it_ext)->_check_param(*it_int, i)
                  == true){
               one_matched = true;
               break;
            }
         ...
         if( one_matched == false ){
            ...
            possible_subprogram_decls->erase( *it_ext );
            break;
         }
      ...
      if( possible_return_types != NULL ){
         // This function (resolve_subprogram_decls) isn't supposed to
         // modify the set of possible return types.
         savant::set<IIRScram_TypeDefinition*> temp_return_types( *possible_return_types );
         reconcile_sets( possible_subprogram_decls, &temp_return_types );
      }
   }
}
...
resolve_subprogram_decls( savant::set<IIRScram_Declaration*>     *possible_subprogram_decls,
   ...
   savant::set<IIRScram_TypeDefinition*> return_set( return_type );
   dl_list<savant::set<IIRScram_TypeDefinition*> > signature_list;
   ...
   while( current_type_def != NULL ){
      savant::set<IIRScram_TypeDefinition*> *current_typedef_set;

      current_typedef_set = new savant::set<IIRScram_TypeDefinition*>( current_type_def );
...
find_incomplete_type_declaration( IIR_Identifier *type_name ){
   ...
   savant::set<IIR_TypeDeclaration*> *incomplete_types = sym_tab->get_incomplete_types();
   IIR_TypeDeclaration *current = NULL;
   for(auto it = incomplete_types->begin();
         it != incomplete_types->end(); it++)
   {
      ASSERT( (*it)->get_declarator() != NULL );
      if( IIRBase_TextLiteral::cmp( (*it)->get_declarator(),
               type_name ) == 0 ){
         found_one = true;
         current = *it;
         break;
